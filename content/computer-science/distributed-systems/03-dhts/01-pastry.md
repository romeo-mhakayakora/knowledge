...
|---|---|---|
| `insert(key, value)` | Store the pair | $pprox \Theta(1)$ |
| `lookup(key)` | Return value, or null | $pprox \Theta(1)$ |
| `delete(key)` | Remove the pair | $pprox \Theta(1)$ |
...
**Pastry's headline guarantee:** routing completes in $\lceil \log_{2^b}(N) ceil$ steps. With $b=4$ and $N=1{,}000{,}000$ nodes: $\log_{16}(10^6) = \log_{16}(16^5) = 5$ steps. Five hops to find anything among a million nodes.
...
The neighborhood set $\mathcal{M}$ contains $M pprox 2^{b+1}$ nodes that are **geographically or topologically close** to the current node — e.g., nodes in the same data center, same ISP, or nearby in latency.
...
**Routing complexity:** $O(\log_{2^b} N)$ hops. With $b=4$ and $N=100{,}000$: $\log_{16}(100{,}000) pprox 4$ hops. Experimentally confirmed:
...
$$P = (1 - 16^{-m})^N = (1 - 16^{-c} \cdot 16^{-\log_{16}(N)})^N = \left(1 - rac{16^{-c}}{N}ight)^N$$

Let $\lambda = 16^{-c}$. Rewrite:

$$P = \left(1 - rac{\lambda}{N}ight)^N = \left[\left(1 - rac{\lambda}{N}ight)^{N/\lambda}ight]^\lambda$$

Applying the standard limit $\lim_{N 	o \infty}(1 - \lambda/N)^{N/\lambda} = e^{-1}$:

$$oxed{P = e^{-\lambda} = e^{-16^{-c}}}$$
...
|---|---|---|
| 0 | 1 | $e^{-1} pprox 0.37$ |
| 1 | $1/16 pprox 0.063$ | $pprox 0.94$ |
| 2 | $1/256 pprox 0.004$ | $pprox 0.996$ |
| 3 | $pprox 2.4	imes10^{-4}$ | $pprox 0.9998$ |
...
So $m = c + 5 pprox 7$ hops at most — even for a million-node network.
